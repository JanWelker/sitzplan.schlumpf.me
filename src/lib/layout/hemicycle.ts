 ...
 *
 * Seats are rendered as plain axis-aligned squares (see SeatChart.svelte) —
 * no per-seat rotation. An earlier version rotated each seat tangentially to
 * its row's curve, which reads as a "twisted" diamond wherever a row curves
 * sharply (the outer arcs), even though the underlying position is correct.
 * Matching the official parlament.ch style avoids that entirely.
 */
